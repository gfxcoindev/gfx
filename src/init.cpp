...
void Shutdown()
{
    ...
    RenameThread("graphicscoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: graphicscoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: graphicscoind.pid)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the GraphicsCoin Wiki for SSL setup instructions)") + "\n";
...
/** Sanity checks
 *  Ensure that GraphicsCoin is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.graphicscoin.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GraphicsCoin is shutting down."));
...
#endif
    // Make sure only a single GraphicsCoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GraphicsCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("GraphicsCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "GraphicsCoin server starting\n");
    ...
    // requested to kill graphicscoin-qt during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of GraphicsCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart GraphicsCoin to complete") << "\n";
