...
#if defined(NDEBUG)
# error "GraphicsCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "GraphicsCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nHeight, int64_t nFees)
{
    ...
    if(nHeight == 4)
    {
        nSubsidy = 140000000 * COIN; // 4mill Premine
    }
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal graphicscoin miner
...
{
    RenameThread("graphicscoin-loadblk");
...
int64_t GetDevOpsPayment(int nHeight, int64_t blockValue)
{
    ...
    ret2 = 0; // Will modify when required use case is solid
    return ret2;
}
