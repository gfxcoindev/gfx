/*
 * Qt4 graphicscoin GUI.
 *
 * W.J. van der Laan 2011-2012
 * The GraphicsCoin Developers 2011-2012
 */
...
{
    resize(850+95, 550);
    setWindowTitle(tr("GraphicsCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/graphicscoin"));
    setWindowIcon(QIcon(":icons/graphicscoin"));
#else
...
void EraGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a GraphicsCoin address"));
    ...
    aboutAction = new QAction(tr("&About GraphicsCoin"), this);
    aboutAction->setToolTip(tr("Show information about GraphicsCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for GraphicsCoin"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/graphicscoin"), tr("&Show / Hide"), this);
...
void EraGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("GraphicsCoin client") + QString(" ") + tr("[testnet]"));
...
void EraGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("GraphicsCoin client"));
...
void EraGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to GraphicsCoin network", "", count));
}
...
{
    QString strTitle = tr("GraphicsCoin") + " - ";
...
void EraGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid GraphicsCoin address or malformed URI parameters."));
    }
