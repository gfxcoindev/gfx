...
bool parseEraURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("graphicscoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!EraUnits::parse(EraUnits::GFX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert graphicscoin:// to graphicscoin:
    //
    //    Cannot handle this later, because graphicscoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("graphicscoin://"))
    {
        uri.replace(0, 12, "graphicscoin:");
    }
...
void openConfigfile()
{
    ...
    /* Open graphicscoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GraphicsCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for GraphicsCoin.lnk
...
{
    return GetAutostartDir() / "graphicscoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a graphicscoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=GraphicsCoin\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("GraphicsCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  graphicscoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("GraphicsCoin-Qt"));
