...
const int GFX_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString GFX_IPC_PREFIX("graphicscoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(GFX_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(GFX_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(GFX_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click graphicscoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start graphicscoin: click-to-pay handler");
    else
...
{
    // clicking on graphicscoin: URLs creates FileOpen events on the Mac:
