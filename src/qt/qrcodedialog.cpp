...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("GFX")
    updateDisplayUnit();
}
...
{
    QString ret = QString("graphicscoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GFX unit input in lnReqAmount, we generate the URI with GFX as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(EraUnits::format(EraUnits::GFX, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
